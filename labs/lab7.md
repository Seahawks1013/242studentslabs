...
> Your answer: Bellman-Ford's time is significantly slower than Dijkstra's across all three densities. in the case of E=600, Bellman-Ford takes 10,237 ms compared to Dijkstra's 0.264 ms. This is a 40x difference. At the medium case (E=5,000), the gap is about 113x. At the dense case (E=50,000), the Bellman-Ford is nearly 400x slower. This is explained directly by the complexity formulas. Dijkstra runs in O((V + E) log V). With V = 500 and E = 50,000, that's roughly (50,500) × 9 ≈ 454,500 operations. Bellman-Ford runs in O(VE) = 500 × 50,000 = 25,000,000 operations (about 55× more). 
...
> Your answer: I would choose Dijkstra (it is the clear choice). Since all edge weights are positive, it will be able to handle weighted shortest paths optimally.  With its complexity, (O((V + E) log V) = O((100,000 + 300,000) × 17)), it would have 6.8 million operations per query. This is fast enough for the 50 ms target, especially on modern hardware. It also uses only O(V) extra memory.
